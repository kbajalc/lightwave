...
void dblist(void), rlist(void), alist(void), info(void), fetch(void),
    force_unique_signames(void), print_file(char *filename), jsonp_end(void),
    lwpass(void), lwfail(char *error_message), pnwcheck(void),
...
void lwpass() { printf("  \"success\": true\n}\n"); }
