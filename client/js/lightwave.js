...
(function () {
	...
	var server,
		...
		ady1, // arrow height
		x_mark_str = ""; // raw time string from ?x= URL param; empty = not set
	...
	function show_summary() {
		...
		if (nann > 0) {
			for (ia = 0; ia < nann; ia++) {
				itext +=
					'<tr><td style="vertical-align: top">Annotator: ' +
					ann[ia].name +
					"</td><td>" +
					"(" +
					ann[ia].annotation.length +
		...
		if ($("#viewann").prop("checked")) {
			...
			for (ia = 0; ia < nann; ia++) {
				if (ann[ia].state === 0) {
					...
					a = ann[ia].annotation;
					for (
						i = ann_after(a, t0_ticks);
						0 <= i && i < a.length && a[i].t <= tf_ticks;
						i++
					) {
						atext +=
							"<tr><td>" +
			...
			if (signals) {
				...
				for (t = t0_ticks; t < tf_ticks; t++) {
					...
					for (j = 0; j < is; j++) {
						...
						if (t % sig[j].tps === 0) {
							if (t >= sig[j].tf) {
								sig[j] = find_trace(db, record, sig[j].name, sig[j].tf);
							}
	...
	function slist(t0_string) {
		...
		m = null;
		nsig = 0;
		url = server + "?action=info&db=" + db + "&record=" + record + server_flags;
		show_status(true);
		get_jsonp(url, function (data) {
			...
			if (x_mark_str !== "") {
				// position window so x is 5 s into view (clamp to record start)
				t0_string = timstr(Math.max(0, strtim(x_mark_str) - 5 * tickfreq));
			}
			if (t0_string !== "") {
				t = strtim(t0_string);
			} else {
	...
	function sync_edits() {
		...
		scribe = $("[name=scribe]").val();
		$("#syncnote").html(
			"<p>Waiting for edit backup (sending to " + scribe + ") ...",
		);
		$.ajax({
			contentType: "multipart/form-data",
			data: body,
	...
	function show_plot() {
		...
		if (g_visible === 0) {
			...
			x0s =
				(tscl - Math.floor(((t0_ticks % tickfreq) * tscl) / tickfreq)) % tscl;
			x0r = x0s % griddx;
			x0q = Math.floor(x0s / griddx) * griddx;

			grd +=
				"<title>(click to hide grid)</title></g>" +
				'<path stroke="rgb(241 183 179)" fill="red" stroke-width="' +
				lwl +
				'" d="M' +
				x0r +
				",0 ";
				...
				for (
					...
					if (a[i].x) {
						...
						if (a[i].a === "N") {
							...
							txt = html_escape(a[i].a);
						}
					}
					if (m_visible) {
						y1 = y - 150;
						sva +=
							'<path stroke="rgb(0,0,200)" stroke-width="' +
							lwl +
							'" fill="blue" + opacity="' +
							m_visible +
							'" d="M' +
					...
					sva +=
						'<text x="' + x + '" y="' + y + '" style="text-anchor: middle;"';
					if (ann[ia].state === 2) {
						sva += ' font-weight="bold"';
					}
					sva +=
		...
		for (is = 0; is < nsig; is++) {
			...
			if (trace && s_visible[sname] === 1) {
				...
				// signal is hidden, show label only
				svs +=
					"<title>" +
					html_escape(sname) +
					" (click for highlighted view)</title>" +
		...
		// green x marker — top triangle only (2× blue annotation size), line 2× annotation weight
		svx = "";
		if (x_mark_str !== "") {
			xm = Math.round(((strtim(x_mark_str) - t0_ticks) * tscl) / tickfreq);
			if (xm >= 0 && xm <= svgw) {
				var xlw = lwn * 1.2,        // 1.2× the blue annotation line weight
					xadx = adx2,            // triangle half-width: 2× annotation arrows (total width 4×)
					xady = ady1;            // triangle height: same as annotation arrows
				svx = '<path stroke="limegreen" stroke-width="' + xlw + '" fill="limegreen"'
					+ ' d="M' + xm + ',0'
					+ ' l-' + xadx + ',-' + xady + ' l' + (xadx * 2) + ',0 l-' + xadx + ',' + xady
					+ ' V' + svgh
					+ '" />';
			}
		}

		svg += grd + tst + sva + svx + svs;
	...
	function show_time(x, y) {
		...
		if (y != 0 && editing) {
			xc = x_cursor - 2 * adx4;
			svc =
				'<path stroke="rgb(0,150,0)" stroke-width="' +
				lwn +
				'" fill="none" style="cursor:pointer" d="M' +
				x_cursor +
				",0 l-" +
				adx2 +
				",-" +
	...
	function load_palette(summary) {
		...
		annot = { t: null, a: null, c: null, n: null, s: null, x: null };
		palette = summary;
		imax = summary.length;
		if (imax > 20) {
			imax = 20;
	...
	function srev() {
		...
		if (i >= 0) {
			...
			// no match found, disable further reverse searches
			$(".srev").attr("disabled", "disabled");
			alert(
				target +
				" not found in " +
	...
	function find() {
		...
		if (ia >= nann) {
			alert(
				"No annotations have been chosen for searching!\n\n" +
	...
	function sfwd() {
		...
		if (i < na) {
			...
			// no match found, disable further forward searches
			$(".sfwd").attr("disabled", "disabled");
			alert(
				target + " not found in " + ann[ia].name + " after " + timstr(tf_ticks),
			);
		}
	}
	...
	function highlight_selection() {
		...
		if (selann >= 0) {
			...
			if (0 <= dt && dt <= dt_ticks) {
				...
				y0 = asy0 - 2 * svgf;
				svsa =
					'<path stroke="rgb(0,0,0)" stroke-width="' +
					lwn +
	...
	function copy_template_to_palette() {
		...
		if (i >= palette.length) {
			...
			if (palette[0][1] < 0) {
				...
				// add new entry at head of array
			...
			// if not new, just select key in palette
			f = seltype.split("_");
			if (i !== f[1]) {
				// if key not selected already
	...
	function select_type(e) {
		var annot = { a: null, s: null, c: null, n: null, x: null },
			f,
			s;
	...
	function mark(e) {
		...
		if (selann >= 0) {
			...
			if (
				asy0 - 2 * svgf < y_cursor &&
				y_cursor < asy0 + svgf &&
				asel.t - dt_tol < t_cursor &&
				t_cursor < asel.t + dt_tol
			) {
				in_box = true;
			}
		}
		...
		if (insert_mode) {
			if (asel === null) {
				...
				// change selected annotation without moving it
			...
			} else if (asel.t !== anew.t) {
				// move selected annot without changing it
				edlog(asel, "-"); // remove selected annotation
			...
			// can't delete -- no selection
			alert(
				"Select an annotation to delete it,\n" +
	...
	function alert_close_warning() {
		if (changes.length > undo_count) {
			save_editlog(db, record, annselected);
			return (
				"Your pending edits have been saved in local storage only. " +
	...
	function edlog(annot, etype) {
		...
		if (annot.x && annot.x.length > 0) {
			// fix whitespace in annot.x
	...
	function handle_editmode() {
		...
		if (db !== "" && record !== "") {
			// return to View/edit tab if record open
			$("#tabs").tabs("option", "active", $("#view").index());
			show_plot();
		}
	}
	...
	function new_annset() {
		...
		if (nann === 0) {
			...
			if (ann[0].name.substring(0, 3) === "new") {
				if (ann[0].annotation.length > 0) {
					...
					if (i > 4) {
						// no more than 5 "new" sets per record
						alert(
							"To create additional annotation sets, sync and" +
		...
		svsa = "";
		$("#syncnote").html(
			"Annotation set <b>" + ann[0].name + "</b> has been initialized.",
		);
		$("#tabs").tabs("option", "active", $("#view").index());
		show_plot();
		if (emode === 1) {
	...
	function set_handlers() {
		...
		try {
			...
			$("#edit_mode").controlgroup("disable");
		}
